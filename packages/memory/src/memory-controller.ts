...
import type { EventStore, MemoryLayer } from "@civaapple/qi-kernel";
import { createId, parseSessionEvent, type MemoryId, type SessionEvent, type SessionId } from "@civaapple/qi-protocol";
import { SqliteMemoryIndex, type IndexedMemoryClaim } from "./memory-index.js";
...
export class MemoryController {
  ...
  retrieve(options: Parameters<SqliteMemoryIndex["search"]>[0]): IndexedMemoryClaim[] {
    return this.#index.search(options);
  }

  contextBlocks(options: Parameters<SqliteMemoryIndex["search"]>[0], priority = 60): ContextBlock[] {
