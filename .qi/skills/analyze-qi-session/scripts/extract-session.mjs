...
Database resolution for --workspace-root (first existing path wins):
  1. $QI_HOME/projects/<workspace-slug>/qi.sqlite   (TUI default)
  2. <workspace>/.qi/qi.sqlite                     (legacy / local)
--workspace is accepted as an alias of --workspace-root (same placement rules).
--project uses $QI_HOME/projects/<slug>/qi.sqlite directly.
--url may include ?project=<slug> for local SQLite fallback when Web is down.
...
/** Cursor-style Workspace slug used under QI_HOME/projects/<slug>. */
...
export function candidateSessionDatabases({
  ...
  if (slug) {
    candidates.push({
      kind: "qi-home",
      path: resolve(defaultQiHome(environment, homeDirectory), "projects", slug, "qi.sqlite"),
    });
  }
  if (workspace) {
    candidates.push({
      kind: "workspace-local",
      path: resolve(workspace, ".qi", "qi.sqlite"),
    });
  }
  return candidates;
}
...
/**
 * Resolve the Session SQLite path. Prefers QI_HOME/projects/<slug> (TUI default)
 * over <workspace>/.qi when both could apply.
 */
...
function absoluteWorkspacePath(workspaceRoot) {
